"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.FirmaController = void 0;
const common_1 = require("@nestjs/common");
const current_user_decorator_1 = require("../common/decorators/current-user.decorator");
const firmar_dto_1 = require("./dto/firmar.dto");
const firma_service_1 = require("./firma.service");
let FirmaController = class FirmaController {
    firma;
    constructor(firma) {
        this.firma = firma;
    }
    documentos(user) {
        return this.firma.listar(user);
    }
    validar(user, dto) {
        return this.firma.validarCertificado(user, dto.password);
    }
    firmarOficio(user, id, dto) {
        return this.firma.firmarOficio(user, id, dto.password);
    }
    firmarDocumento(user, id, dto) {
        return this.firma.firmarDocumento(user, id, dto.password);
    }
};
exports.FirmaController = FirmaController;
__decorate([
    (0, common_1.Get)('documentos'),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object]),
    __metadata("design:returntype", void 0)
], FirmaController.prototype, "documentos", null);
__decorate([
    (0, common_1.Post)('validar'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, firmar_dto_1.FirmarDto]),
    __metadata("design:returntype", void 0)
], FirmaController.prototype, "validar", null);
__decorate([
    (0, common_1.Post)('oficios/:id'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('id', common_1.ParseIntPipe)),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number, firmar_dto_1.FirmarDto]),
    __metadata("design:returntype", void 0)
], FirmaController.prototype, "firmarOficio", null);
__decorate([
    (0, common_1.Post)('documentos/:id'),
    (0, common_1.HttpCode)(common_1.HttpStatus.OK),
    __param(0, (0, current_user_decorator_1.CurrentUser)()),
    __param(1, (0, common_1.Param)('id', common_1.ParseIntPipe)),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Number, firmar_dto_1.FirmarDto]),
    __metadata("design:returntype", void 0)
], FirmaController.prototype, "firmarDocumento", null);
exports.FirmaController = FirmaController = __decorate([
    (0, common_1.Controller)('firma'),
    __metadata("design:paramtypes", [firma_service_1.FirmaService])
], FirmaController);
//# sourceMappingURL=firma.controller.js.map